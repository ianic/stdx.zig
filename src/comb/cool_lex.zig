...
test "CoolLexSlice" {
    ...
    try std.testing.expectEqualSlices(u8, &test_data_5_3[0], &a);

    var j: usize = 1;
    while (cl.next()) : (j += 1) {
        // call next and then visit another combination
        try std.testing.expectEqualSlices(u8, &test_data_5_3[j], &a);
    }
    // next returns false
